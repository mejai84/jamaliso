...
async function applyStep9() {
    console.log('--- APLICANDO CONFIGURACION DE PROPINAS ---');

    const sql = fs.readFileSync(path.resolve('supabase_migrations/step9-tips-config.sql'), 'utf8');
    ...
    if (response.ok) {
        console.log('✅ Migración paso 9 aplicada.');
    } else {
        const err = await response.json();
        console.error('❌ Error applying step 9:', err);
    }
}
