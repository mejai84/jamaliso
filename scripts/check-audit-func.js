...
async function checkFunction() {
    const { data: res } = await supabase.rpc('query_sql', {
        query_text: `SELECT routine_definition FROM information_schema.routines WHERE routine_name = 'log_audit_event'`
    });
    console.log('Function log_audit_event:', res?.[0]?.routine_definition);
}
