...
async function checkCols() {
    const { data: pCols } = await supabase.rpc('query_sql', {
        query_text: "SELECT column_name FROM information_schema.columns WHERE table_name = 'payments'"
    });
    console.log('Payments Columns:', pCols);

    const { data: mCols } = await supabase.rpc('query_sql', {
        query_text: "SELECT column_name FROM information_schema.columns WHERE table_name = 'cash_movements'"
    });
    console.log('Movements Columns:', mCols);
}
