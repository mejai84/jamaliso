...
async function checkRestaurantCols() {
    const { data, error } = await supabase.rpc('query_sql', {
        query_text: "SELECT column_name, data_type FROM information_schema.columns WHERE table_name = 'restaurants' AND table_schema = 'public'"
    });
    console.log('Restaurant Columns:', data);
}
