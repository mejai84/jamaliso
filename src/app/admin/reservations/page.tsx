...
export default function AdminReservationsPage() {
    ...
    if (loading) return (
        <div className="min-h-screen bg-black flex flex-col items-center justify-center p-8 gap-4">
            ...
            <p className="font-black italic uppercase text-[10px] tracking-[0.3em] text-gray-500">Sincronizando Calendario...</p>
        </div>
    )
        ...
        <div className="min-h-screen bg-black text-white p-4 md:p-8 selection:bg-primary font-sans">
            <div className="max-w-[1400px] mx-auto space-y-10">
                ...
                <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-8 animate-in fade-in slide-in-from-top-4 duration-700">
                    <div className="space-y-2">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h1 className="text-4xl font-black tracking-tighter uppercase italic leading-none">Gestión de <span className="text-primary">Reservas</span></h1>
                                <p className="text-[10px] text-gray-500 font-bold uppercase tracking-[0.2em] mt-2 italic flex items-center gap-2">
                            ...
                            {['upcoming', 'pending', 'confirmed', 'all'].map((f) => (
                                <button
                                    ...
                                    className={cn(
                                        ...
                                        filter === f ? "bg-primary text-black" : "text-gray-500 hover:text-white"
                                    )}
                                >
                ...
                <div className="relative group max-w-2xl">
                    ...
                    <input
                        ...
                        className="w-full h-16 pl-16 pr-6 rounded-[2rem] bg-[#111] border border-white/5 focus:border-primary focus:outline-none font-black italic text-sm tracking-tight transition-all"
                    />
                </div>
                ...
                <div className="grid grid-cols-1 gap-4 animate-in fade-in slide-in-from-bottom-4 duration-700">
                    {filteredReservations.length > 0 ? (
                        filteredReservations.map(reservation => (
                            <div
                                key={reservation.id}
                                className="group bg-[#111] rounded-[2.5rem] p-8 border border-white/5 hover:border-primary/20 hover:bg-black/40 transition-all flex flex-col lg:flex-row lg:items-center justify-between gap-8 relative overflow-hidden"
                            >
                                {/* Date Badge */}
                                <div className="flex gap-8 items-center relative z-10">
                                    <div className="flex flex-col items-center justify-center w-20 h-20 bg-black rounded-3xl border border-white/5 shadow-2xl group-hover:border-primary/20 transition-all">
                                        <span className="text-[10px] font-black uppercase text-gray-500 tracking-widest">{new Date(reservation.reservation_date + 'T12:00:00').toLocaleDateString('es-CO', { month: 'short' })}</span>
                                    ...
                                    <div className="space-y-4">
                                        <div className="flex flex-wrap items-center gap-3">
                                            <h3 className="text-2xl font-black italic uppercase tracking-tighter group-hover:text-primary transition-colors line-clamp-1">{reservation.customer_name}</h3>
                                            <span className={cn(
                                ...
                                <div className="flex flex-wrap items-center gap-3 relative z-10 pt-6 lg:pt-0 border-t lg:border-t-0 border-white/5">
                                    ...
                                    <div className="flex gap-2 ml-4">
                                        <Button onClick={() => sendWhatsApp(reservation, 'confirm')} variant="ghost" size="icon" className="h-12 w-12 rounded-xl bg-white/5 border border-white/5 text-gray-500 hover:text-emerald-500 hover:border-emerald-500/30">
                                            <Share2 className="w-4 h-4" />
                                        </Button>
                                        <Button onClick={() => handleDelete(reservation.id)} variant="ghost" size="icon" className="h-12 w-12 rounded-xl bg-white/5 border border-white/5 text-gray-500 hover:text-rose-500 hover:border-rose-500/30">
                        ...
                        <div className="text-center py-24 bg-[#111] rounded-[3rem] border border-white/5 border-dashed space-y-4">
                            <CalendarIcon className="w-16 h-16 text-gray-800 mx-auto mb-2" />
                            <h4 className="text-xl font-black italic uppercase tracking-tighter">Sin registros activos</h4>
                            <p className="text-gray-500 font-medium italic">No se encontraron reservas para el filtro seleccionado.</p>
            ...
            {showNewModal && (
                <div className="fixed inset-0 bg-black/90 backdrop-blur-xl z-[100] flex items-center justify-center p-4">
                    <div className="bg-[#111] border border-white/10 p-10 rounded-[3rem] w-full max-w-2xl animate-in zoom-in duration-300 shadow-3xl">
                        <div className="flex items-center justify-between mb-10">
                            <div>
                                ...
                                <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest mt-1">Ingreso manual de mesa al sistema</p>
                            </div>
                            <Button variant="ghost" size="icon" onClick={() => setShowNewModal(false)} className="rounded-full hover:bg-white hover:text-black shrink-0">
                                <X className="w-6 h-6" />
                            </Button>
                        </div>
                        ...
                        <form onSubmit={handleCreateReservation} className="grid grid-cols-2 gap-6">
                            <div className="col-span-2 space-y-3">
                                <label className="text-[10px] font-black uppercase tracking-widest text-gray-500 ml-1">Cliente</label>
                                <input name="name" required placeholder="Nombre completo" className="w-full h-14 bg-black border border-white/5 rounded-2xl px-6 outline-none text-white focus:border-primary font-bold italic" />
                            </div>
                            <div className="space-y-3">
                                <label className="text-[10px] font-black uppercase tracking-widest text-gray-500 ml-1">Teléfono</label>
                                <input name="phone" required placeholder="Ej: 300 123 4567" className="w-full h-14 bg-black border border-white/5 rounded-2xl px-6 outline-none text-white focus:border-primary font-black italic" />
                            </div>
                            <div className="space-y-3">
                                <label className="text-[10px] font-black uppercase tracking-widest text-gray-500 ml-1">Comensales</label>
                                <input name="guests" type="number" min="1" defaultValue="2" required className="w-full h-14 bg-black border border-white/5 rounded-2xl px-6 outline-none text-white focus:border-primary font-black italic" />
                            </div>
                            <div className="space-y-3">
                                <label className="text-[10px] font-black uppercase tracking-widest text-gray-500 ml-1">Fecha</label>
                                <input name="date" type="date" required className="w-full h-14 bg-black border border-white/5 rounded-2xl px-6 outline-none [color-scheme:dark] text-white focus:border-primary font-black italic" />
                            </div>
                            <div className="space-y-3">
                                <label className="text-[10px] font-black uppercase tracking-widest text-gray-500 ml-1">Hora</label>
                                <input name="time" type="time" required className="w-full h-14 bg-black border border-white/5 rounded-2xl px-6 outline-none [color-scheme:dark] text-white focus:border-primary font-black italic" />
                            </div>
                            <div className="col-span-2 space-y-3">
                                <label className="text-[10px] font-black uppercase tracking-widest text-gray-500 ml-1">Observaciones</label>
                                <textarea name="notes" placeholder="Mesa especial, alergias, decoraciones..." className="w-full h-28 bg-black border border-white/5 rounded-2xl px-6 py-4 outline-none resize-none text-white focus:border-primary font-bold italic" />
                            </div>
...
function ReservationKPI({ label, value, color, icon }: any) {
    return (
        <div className="bg-[#111] border border-white/5 p-8 rounded-[2rem] shadow-2xl relative overflow-hidden group">
            ...
            <div className="relative z-10 space-y-1">
                <p className="text-[9px] font-black uppercase tracking-[0.3em] text-gray-500 italic">{label}</p>
