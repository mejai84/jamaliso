...
export default function PargoHubPage() {
    ...
    if (loading) return (
        <div className="min-h-screen bg-black flex flex-col items-center justify-center p-8 gap-4">
            ...
            <p className="font-black italic uppercase text-[10px] tracking-[0.4em] text-gray-500">Sincronizando Pargo Hub...</p>
        </div>
    )
    ...
    return (
        <div className="min-h-screen bg-[#050505] text-white font-sans selection:bg-primary pb-20 overflow-x-hidden">
            ...
            {/* 📱 MOBILE HEADER */}
            <header className="sticky top-0 z-50 bg-black/60 backdrop-blur-xl border-b border-white/5 p-6 flex items-center justify-between">
                <div className="flex items-center gap-4">
                    <Link href="/admin">
                        <Button variant="ghost" size="icon" className="h-12 w-12 rounded-2xl bg-white/5 border border-white/10">
                            <ArrowLeft className="w-5 h-5" />
                        </Button>
                    </Link>
                    <div className="flex items-center gap-3">
                        {businessInfo.logo ? (
                            <img src={businessInfo.logo} alt="Logo" className="w-10 h-10 rounded-xl object-cover border border-white/10" />
                        ) : (
                        ...
                        <div>
                            <h1 className="text-xl font-black tracking-tighter uppercase italic leading-none">{businessInfo.name}</h1>
                            <div className="flex items-center gap-1.5 mt-1">
                                <div className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" />
                                <span className="text-[8px] font-black uppercase tracking-widest text-emerald-400">Live Dashboard</span>
                            </div>
                        </div>
                    </div>
                </div>
                <div className="flex items-center gap-3">
                    <button
                        ...
                        className={cn("p-3 bg-white/5 border border-white/10 rounded-2xl transition-all active:scale-90", refreshing && "animate-spin text-primary")}
                    >
                        <RefreshCcw className="w-5 h-5" />
                    </button>
                    <div className="p-3 bg-white/5 border border-white/10 rounded-2xl relative">
            ...
            <main className="relative z-10 p-6 space-y-8 max-w-lg mx-auto">
                ...
                {/* 💰 MAIN KPI: REVENUE */}
                <div className="bg-[#0a0a0a] border border-white/10 rounded-[3rem] p-10 shadow-3xl relative overflow-hidden group">
                    ...
                    <div className="relative z-10 text-center">
                        ...
                        <h2 className="text-6xl font-black tracking-tighter italic leading-none">{formatPrice(stats.revenue)}</h2>
                        <div className="mt-8 pt-8 border-t border-white/5 flex justify-center gap-8">
                            <div className="text-center">
                                <p className="text-[8px] font-black text-gray-500 uppercase tracking-widest mb-1 italic">Ticket Prom.</p>
                                <p className="text-xl font-black italic tracking-tighter">{formatPrice(stats.avgTicket)}</p>
                            </div>
                            <div className="w-px h-10 bg-white/10 self-center" />
                            <div className="text-center">
                                <p className="text-[8px] font-black text-gray-500 uppercase tracking-widest mb-1 italic">Ventas Totales</p>
                                <p className="text-xl font-black italic tracking-tighter">{stats.orders}</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-[#0a0a0a] border border-white/10 rounded-[2.5rem] p-8 space-y-6">
                    <div className="flex justify-between items-center">
                        <h3 className="text-[10px] font-black text-gray-500 uppercase tracking-[0.4em] italic flex items-center gap-2">
                            <TrendingUp className="w-3 h-3 text-orange-500" /> Picos de Demanda
                        </h3>
                        <span className="text-[8px] font-black text-gray-600 uppercase tracking-widest bg-white/5 px-2 py-1 rounded-lg">Tendencia 30d</span>
                    </div>

                    <div className="h-24 flex items-end gap-1.5 px-2">
                        {peakHours.map((p, i) => (
                            <div key={i} className="flex-1 group relative flex flex-col items-center">
                                <div className="opacity-0 group-hover:opacity-100 absolute bottom-full mb-2 bg-black border border-white/10 p-2 rounded-lg text-[8px] font-black text-white whitespace-nowrap z-50 transition-opacity">
                                ...
                                <div
                                    className={cn(
                                        ...
                                        p.intensity > 80 ? "bg-orange-600 shadow-[0_0_10px_rgba(234,88,12,0.3)]" :
                                            p.intensity > 40 ? "bg-primary/60" : "bg-white/10"
                                    )}
                                    style={{ height: `${Math.max(15, p.intensity)}%` }}
                                />
                                <span className="text-[6px] font-black text-gray-700 mt-2">{p.hour}h</span>
                            </div>
                        ))}
                        {peakHours.length === 0 && <p className="w-full text-center text-[8px] font-black text-gray-700 uppercase italic py-8">Calculando datos de afluencia...</p>}
                    </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div className="bg-[#0a0a0a] border border-white/5 rounded-[2.5rem] p-6 space-y-4">
                        ...
                        <div>
                            <p className="text-[8px] font-black text-gray-500 uppercase tracking-widest italic">Ocupación</p>
                            <p className="text-2xl font-black italic tracking-tighter">{stats.activeTables} <span className="text-[10px] text-gray-600">Mesas</span></p>
                        </div>
                    </div>
                    <div className="bg-[#0a0a0a] border border-white/5 rounded-[2.5rem] p-6 space-y-4">
                        ...
                        <div>
                            <p className="text-[8px] font-black text-gray-500 uppercase tracking-widest italic">Estado Caja</p>
                            <p className={cn(
                                "text-sm font-black italic tracking-tighter uppercase",
                                stats.cashboxStatus === 'OPEN' ? "text-emerald-400" : "text-rose-500"
                            )}>
                ...
                <div className="space-y-6">
                    ...
                    <div className="space-y-3">
                        {recentSales.map((sale) => (
                            <div key={sale.id} className="bg-[#0a0a0a] border border-white/5 rounded-[2rem] p-5 flex items-center justify-between group active:scale-[0.98] transition-all">
                                <div className="flex items-center gap-4">
                                    <div className="w-12 h-12 rounded-2xl bg-white/5 border border-white/5 flex items-center justify-center font-black text-primary text-[10px] italic">
                                        #{sale.id.split('-')[0].toUpperCase()}
                                    </div>
                                    <div>
                                        <p className="text-[10px] font-black uppercase italic tracking-tighter text-white">{sale.guest_info?.name || 'Venta Rápida'}</p>
                                ...
                                <div className="text-right">
                                    <p className="text-xl font-black italic tracking-tighter text-emerald-400">{formatPrice(sale.total)}</p>
                                    <div className="flex items-center justify-end gap-1 mt-0.5">
                                        <div className="w-1 h-1 rounded-full bg-emerald-500" />
                                        <span className="text-[7px] font-black uppercase text-emerald-500 opacity-60">Success</span>
                                    </div>
                                </div>
                            </div>
                        ))}
                ...
                <div className="bg-primary text-black rounded-[2.5rem] p-8 shadow-3xl flex items-center justify-between">
                    <div className="flex items-center gap-4">
                    ...
                    <Button variant="ghost" className="h-12 w-12 rounded-2xl bg-black text-white p-0 flex items-center justify-center">
            ...
            <nav className="fixed bottom-0 left-0 w-full h-20 bg-black/80 backdrop-blur-xl border-t border-white/5 flex items-center justify-around px-6 z-50">
