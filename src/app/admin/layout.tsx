...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
    ...
    return (
        <div className="flex min-h-screen bg-black selection:bg-primary selection:text-black">
            {/* 🏰 ENTERPRISE SIDEBAR */}
            <aside className="w-72 border-r border-white/5 bg-[#050505] hidden lg:flex flex-col sticky top-0 h-screen">
                ...
                <div className="p-8 pb-4 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col">
                            <span className="text-sm font-black italic uppercase tracking-tighter text-white">PARGO <span className="text-primary italic">OS</span></span>
                            <span className="text-[8px] font-bold text-gray-500 tracking-[0.2em] uppercase">Enterprise v1.6</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="px-6 py-4 mb-4">
                    <div className="p-4 rounded-3xl bg-white/5 border border-white/5 flex items-center gap-4">
                        <div className="w-10 h-10 rounded-full bg-primary/20 flex items-center justify-center border border-primary/20 font-black text-primary italic">
                            {userName.charAt(0)}
                        </div>
                        <div className="flex flex-col overflow-hidden">
                            <span className="text-xs font-black italic text-white uppercase truncate">{userName}</span>
                ...
                <div className="px-6 mb-6">
                    <div className="relative group">
                        <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gray-600 group-focus-within:text-primary transition-colors" />
                        <input
                            type="text"
                            placeholder="Buscar módulo..."
                            className="w-full h-10 pl-10 pr-4 rounded-xl bg-white/2 border border-white/5 outline-none focus:border-primary/50 text-[10px] font-bold uppercase tracking-widest placeholder:text-gray-700 transition-all"
                        />
                    </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto px-4 custom-scrollbar space-y-8 pb-10">
                    {sidebarSections.map((section, idx) => (
                        <div key={idx} className="space-y-3">
                            <h4 className="px-4 text-[9px] font-black text-gray-700 uppercase tracking-[0.3em] font-mono">{section.title}</h4>
                ...
                <div className="p-6 border-t border-white/5 space-y-4">
                    ...
                    <Button
                        variant="ghost"
                        className="w-full justify-start gap-4 h-12 text-[10px] font-black uppercase italic tracking-widest text-rose-500 hover:bg-rose-500 hover:text-white rounded-[1.5rem] transition-all"
                        onClick={async () => {
            ...
            <main className="flex-1 flex flex-col min-w-0">
                ...
                <header className="h-16 border-b border-white/5 flex items-center px-6 lg:hidden justify-between bg-[#050505]/80 backdrop-blur-md sticky top-0 z-50">
                    <div className="flex items-center gap-3">
                        <Zap className="w-5 h-5 text-primary" />
                        <span className="font-black italic text-sm tracking-tighter uppercase">PARGO <span className="text-primary italic">OS</span></span>
                    </div>
                    <Button
                        size="icon"
                        variant="ghost"
                        className="rounded-xl border border-white/5 active:scale-90 transition-transform"
                ...
                {isMobileMenuOpen && (
                    <div className="fixed inset-0 z-[100] lg:hidden">
                        ...
                        />
                        <aside className="absolute left-0 top-0 bottom-0 w-[280px] bg-[#050505] border-r border-white/10 flex flex-col animate-in slide-in-from-left duration-300 shadow-2xl">
                            ...
                            <div className="flex-1 overflow-y-auto p-4 space-y-8 custom-scrollbar">
                                {sidebarSections.map((section, idx) => (
                                    <div key={idx} className="space-y-3">
                                        <h4 className="px-4 text-[8px] font-black text-gray-700 uppercase tracking-[0.3em]">{section.title}</h4>
                                        <div className="space-y-1">
                                            {section.items
                                                .filter(item => userRole === 'admin' || item.roles.includes(userRole))
                                                .map((item) => {
                                                    ...
                                                    return (
                                                        ...
                                                        >
                                                            <div className={cn(
                                                                ...
                                                                isActive ? "bg-primary text-black" : "text-gray-500 hover:text-white"
                                                            )}>
                            ...
                            <div className="p-6 border-t border-white/5">
                                <Button
                                    variant="ghost"
                                    className="w-full justify-start gap-4 h-12 text-[10px] font-black uppercase italic tracking-widest text-rose-500"
                                    onClick={async () => {
                                        await supabase.auth.signOut()
                                        router.push("/login")
                                    }}
