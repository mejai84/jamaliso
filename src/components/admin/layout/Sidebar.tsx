...
export function Sidebar({ restaurant, userRole, userName }: SidebarProps) {
    ...
    return (
        <aside className="w-72 border-r-2 border-slate-200 bg-white hidden lg:flex flex-col sticky top-0 h-screen z-50 relative font-sans shadow-xl">
            {/* Brand Header */}
            ...
            {/* Business Selector */}
            <div className="px-6 mt-8 mb-6 relative z-50">
                <BusinessSelector />
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-4 custom-scrollbar space-y-12 pb-10 relative z-10">
                        ...
                        <div key={idx} className="space-y-4 font-sans">
                            ...
                            <div className="space-y-1">
                                {filteredItems.map((item) => {
                                    ...
                                    return (
                                        <Link key={item.href} href={item.href} className="block group relative px-2">
                                            <div className={cn(
                                                "flex items-center gap-4 px-4 py-3.5 rounded-2xl transition-all duration-300 relative group/item border-2",
                                                isActive
            ...
            <div className="p-6 pb-8 border-t-2 border-slate-100 bg-slate-50/50 relative z-10 space-y-6">
                <div className="flex items-center gap-4 group cursor-default p-2 bg-white rounded-2xl border-2 border-slate-100">
                    <div className="relative">
                        <div className="w-10 h-10 rounded-xl border-2 border-slate-200 overflow-hidden relative z-10 flex items-center justify-center bg-slate-900 text-white shadow-md">
                            <span className="font-black italic text-base uppercase">{userName.charAt(0)}</span>
                        </div>
                    </div>
                    <div className="flex flex-col">
                        <span className="text-[11px] font-black text-slate-900 tracking-tight uppercase italic truncate max-w-[140px]">{userName}</span>
                        <div className="flex items-center gap-1.5 mt-0.5">
                            <div className="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-pulse" />
                            <span className="text-[8px] font-black text-slate-400 uppercase tracking-[0.2em]">{userRole}</span>
                        </div>
                    </div>
                </div>

                <button
                    className="w-full h-12 bg-white border-2 border-rose-100 text-[9px] font-black uppercase italic tracking-[0.3em] text-rose-500 hover:bg-rose-500 hover:text-white hover:border-rose-500 rounded-xl transition-all flex items-center justify-center gap-3 active:scale-95"
